...
import sampleOut from "../../database/schema/sampleOut.schema.js";
...
export const AddSampleInward = catchAsync(async (req, res,next) => {
  const challan = await sampleOut.findOne({ deliveryChallanNo:req.body.challanNo });
  if(!challan){
...
export const InventoryList = catchAsync(async (req, res) => {
  ...
  if (searchParam) {
    matchStage.$or = [
      ...
      { totalAvailableQuantity: { $eq: isNaN(searchParam) ? searchParam : +searchParam } },
      { totalReservedQuantity: { $eq: isNaN(searchParam) ? searchParam : +searchParam } },
      { totalBalanceQuantity: { $eq: isNaN(searchParam) ? searchParam : +searchParam } },
    ];
  }
...
export const ViewProductHistory = catchAsync(async (req, res) => {
 ...
 if (req?.body?.filters && req?.body?.filters.invoiceDate && req?.body?.filters.invoiceDate.to && req?.body?.filters.invoiceDate.from) {
...
if (req?.body?.filters && req?.body?.filters.receiveDate && req?.body?.filters.receiveDate.to && req?.body?.filters.receiveDate.from) {
  filter.receivedDate = {
