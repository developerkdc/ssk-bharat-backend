...
retailerRouter.route('/')
    ...
    .post(retailer.AddCompany)

retailerRouter.route('/:id')
    .get(retailer.GetCompanyById)
    .patch(retailer.UpdateCompany)

retailerRouter.route("/branch")
    .post(branch.addBranch)

retailerRouter.route("/branch/:companyId")
    .get(branch.getBranchOfCompany)
    .patch(branch.updateBranch)

retailerRouter.route('/branch/contact/:companyId/:branchId')
    .post(branch.AddContact)
    .patch(branch.UpdateContact)

retailerRouter.get('/Branchretailer/all',branch.getAllBranchCompany);

retailerRouter.patch('/branch/upload/:companyId/:branchId', MulterFunction('./uploads/admin/retailerDocument').fields([{ name: 'pan_image', maxCount: 1 }, { name: 'gst_image', maxCount: 1 }, { name: 'passbook_image', maxCount: 1 }]), branch.uploadDocument('./uploads/admin/retailerDocument'));
